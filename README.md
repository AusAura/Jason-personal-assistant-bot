...
(Team version of the application)
...
- Incorporates 3 sub-applications: Address Book, Notebook, File Sorter.
- Address Book is made to be somewhat generic and potentially extensible. It uses currying and command parser that allows adding a command that consists of unlimited numbers of words and with any number of arguments (passed parameters are still limited by specified arguments in the function def).
- Universal decorator is made to hadle most of possible errors.
- Notebook uses prompt-toolkit to show available commands in CLI.
- Both sub-applications save data in JSON locally.
...
**Usage:**
To use the notebook, run "jason" in the command line and select the desired command (1) from the menuю
...
**Usage:**
To use the notebook, run "jason" in the command line and select the desired command (2) from the menu, which provides a command-line interface to interact with your notes. You can add, edit, delete, add tags, search, list, save, and load notes using the supported commands.
